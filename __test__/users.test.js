...
describe('User API Unit Tests', () => {
    describe('POST /create', () => {
        it('Should return a new user on valid inputs and permissions', async () => {
            ...
            const mockNewUser = {
                user_id: '456', name: 'John Doe', role: 'staff',
                email: 'jd@example.com', phone: '12345678',
                password: '00000000', status: 'active'
            };
            ...
            const res = await request(app)
                .post('/create')
                .send({ name: 'John Doe', email: 'jd@example.com', phone: '12345678', password: '00000000' })
                .set('Cookie', ['access_token=valid_access_token']);

            expect(res.status).toBe(201);
            expect(res.body).toEqual({
                user_id: '456', name: 'John Doe', role: 'staff',
                email: 'jd@example.com', phone: '12345678', status: 'active'
            });
        });

        it('Should return 403 error if staff role tries to create user', async () => {
            const mockManager = { user_id: '123', role: 'staff' };
        ...
        it('Should return 400 error for invalid inputs', async () => {
            ...
            const res = await request(app)
                .post('/create')
                .send({ name: 'J', email: 'invalid.email', phone: '12345678', password: '123' })
                .set('Cookie', ['access_token=valid_access_token']);
    ...
    describe('PATCH /change-password', () => {
        it('Should change password', async () => {
            ...
            expect(mockUpdateUser).toHaveBeenCalledWith({
                data: { password: 'hashed_new_password', modified_by: '123' },
                where: { user_id: '123' }
            });
        });
    ...
    describe('PATCH /reset-password', () => {
        it("should allow managers to reset specific user's password.", async () => {
            const mockManager = { user_id: '123', role: 'manager', password: 'hashed_password' };
            const mockUser = { user_id: '456', password: 'hashed_password' };
            ...
            expect(mockUpdateUser).toHaveBeenCalledWith({
                data: { password: 'new_hashed_password', modified_by: '123' },
                where: { user_id: '456' },
    ...
    describe('PATCH /deactivate', () => {
        it('should allow managers to deactivate a specific user account', async () => {
            ...
            const mockUser = {
                ...
                email: 'sample@example.com', phone: '000000', role: 'staff'
            };
            ...
            expect(mockUpdateUser).toHaveBeenCalledWith({
                data: { status: 'inactive', modified_by: '123' },
                where: { user_id: '456' },
                select: { password: false }
            });
        });
