...
export const CreateMilkPool = async (req, res) => {
    ...
    try {
        ...
        const earliest_expiration = new Date(
            Math.min(...raw_milks.map((m) => new Date(m.expiration_date))),
        );
        ...
        for (const milk of raw_milks) {
            if (milk.qat_status !== "pass") {
                return res.status(400).json({
                    error: `Raw milk CTN ${milk.ctn} has a QAT status of '${milk.qat_status}'. Only 'pass' is allowed.`,
                });
            }
            if (milk.pid !== null) {
                return res.status(400).json({
                    error: `Raw milk CTN ${milk.ctn} is already pooled in pool ID ${milk.pid}.`,
                });
            }
            if (milk.milk_status === "discarded") {
                return res.status(400).json({
                    error: `Raw milk CTN ${milk.ctn} has a status of 'discarded' and cannot be pooled.`,
                });
            }
            expected_volume_ml += Number(milk.volume_ml || 0);
        }

        const final_actual_volume = actual_volume_ml
            ? Number(actual_volume_ml)
            : expected_volume_ml;
...
export const UpdatePoolQAT = async (req, res) => {
    try {
        ...
        if (existing_pool.milk_status === "discarded") {
            return res
                .status(400)
                .json({ error: "This pool has already been discarded and cannot be updated." });
        }
