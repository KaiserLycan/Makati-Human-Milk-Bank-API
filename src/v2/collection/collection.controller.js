    ...
    try {
        ...
        if (currentDailyTotal + volume_ml > 800) {
            return res.status(400).json({
                error: `Collection exceeds daily limit. Current total today is ${currentDailyTotal} ml.`,
            });
        }
...
export const UpdateCollection = async (req, res) => {
    try {
        ...
        const { volume_ml, expiration_date, pickup_date, hospital, health_center, remarks } =
            req.body;
        const user_id = req.user.user_id;
...
export const PatchMilkStatus = async (req, res) => {
    try {
        ...
        if (!["good", "contaminated", "discarded", "expired"].includes(milk_status)) {
            return res.status(400).json({
                error: "Invalid milk status. Allowed values are: good, contaminated, discarded, expired.",
            });
        }
...
export const PatchQATStatus = async (req, res) => {
    try {
        ...
        if (!["pending", "pass", "fail"].includes(qat_status)) {
            return res
                .status(400)
                .json({ error: "Invalid QAT status. Allowed values are: pending, pass, fail." });
        }
