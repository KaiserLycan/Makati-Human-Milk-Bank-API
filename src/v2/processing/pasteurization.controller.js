    ...
    try {
        ...
        if (
            ...
            return res.status(400).json({
                error: "All fields required: pid, batch_number, bottle_count, volume_per_bottle, bottle_type, pasteurization_date.",
            });
        }
        ...
        if (total_volume > Number(pool.actual_volume_ml)) {
            return res.status(400).json({
                error: `Total volume (${total_volume} ml) exceeds pool's actual volume (${pool.actual_volume_ml} ml).`,
            });
        }
        ...
        return res.status(201).json({
            message: `Batch logged successfully with ${bottle_count} bottles.`,
...
export const ReportPasteurizationIncident = async (req, res) => {
    try {
        ...
        if (
            milk_status &&
            !["good", "contaminated", "discarded", "expired"].includes(milk_status)
        ) {
            return res.status(400).json({
                error: "Invalid milk status. Must be one of: good, contaminated, discarded, expired.",
            });
        }
        ...
        return res.status(200).json({
            message: "Incident reported and bottle updated successfully.",
...
export const UpdateMBTStatus = async (req, res) => {
    try {
        ...
        if (!["pass", "fail"].includes(mbt_status)) {
            return res
                .status(400)
                .json({ error: "Invalid MBT status. Must be either 'pass' or 'fail'." });
        }
        ...
        return res
            .status(200)
            .json({ message: "MBT status updated successfully.", bottle: updatedBottle });
    } catch (error) {
        if (error.code === "P2025") {
            return res.status(404).json({ error: "Bottle record not found." });
        }
