...
export const runExpirationCheck = async () => {
    ...
    try {
        ...
        for (const milk of expiredRaw) {
            await prisma.raw_milk.update({ where: { ctn: milk.ctn }, data: { milk_status: 'expired' } });
            await notifyStaff('raw_milk', milk.ctn, `Raw milk collection (CTN: ${milk.ctn}) has expired.`);
        }
        ...
        for (const milk of expiredPool) {
            await prisma.pool_milk.update({ where: { pid: milk.pid }, data: { milk_status: 'expired' } });
            await notifyStaff('pool_milk', milk.pid, `Pooled milk batch (PID: ${milk.pid}) has expired.`);
        }
        ...
        for (const milk of expiredPasteurized) {
            await prisma.pasteurized_milk.update({ where: { btl_id: milk.btl_id }, data: { milk_status: 'expired' } });
            await notifyStaff('pasteurized_milk', milk.btl_id, `Pasteurized milk bottle (ID: ${milk.btl_id}) has expired.`);

            // 5. Check if it affects any beneficiary requests (R60)
            for (const rb of milk.request_bottles) {
                const req = rb.request;
                if (req && (req.request_status === 'waiting' || req.request_status === 'allocated')) {
                    ...
                    console.log(`[EMAIL ALERT] Sending cancellation email to ${email} for Request ID: ${req.rid}`);
