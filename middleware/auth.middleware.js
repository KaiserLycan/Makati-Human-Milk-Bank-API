...
export const ProtectRoute = async (req, res, next) => {
    try {
        ...
        if (error.code === "P2025" || error.name ==="JsonWebTokenError" ) return res.status(401).json({error: "Invalid token."});
        if (error.name === "TokenExpiredError") return res.status(401).json({error: "Expired token."});
